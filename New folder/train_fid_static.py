...
def train(model, optimizer, scheduler, step, train_dataset, eval_dataset, opt, collator, best_dev_em, checkpoint_path):
    ...
    for epoch in range(opt.epochs):
        ...
        for i, batch in enumerate(train_dataloader):
            ...
            for j in range(opt.self_consistency_steps):
                # Reconstruct the input from the model's output
                with torch.no_grad():
                    ...
                    # Replace the original input with the reconstructed input
                    context_ids = output_ids.detach()

                # Compute loss on the reconstructed input
                train_loss, _, (loss_tfmc, loss_re) = model(
                    input_ids=context_ids.cuda(),
                    attention_mask=context_mask.cuda(),
                    indices=indices.cuda(),
